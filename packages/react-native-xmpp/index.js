...
var map = {
    ...
    'login': 'RNXMPPLogin',
    'roster': 'RNXMPPRoster',
    'offlineMessage' : 'RNXMPPOfflineMessage'
}
...
class XMPP {
    ...
    connect(username, password, auth = RNXMPP.SCRAMSHA1, hostname = null, port = 5222){
	    ...
	    // username = username.replace(/192.168.80.211/g,'imserver');
			// username = username.replace(/192.168.199.53/g,'gkzq.com.cn');
	    // username = username.replace(/121.30.232.162/g,'imserver');//219.232.203.16 sns.sxzkzy.com
	    username = changeBtn ? username.replace(/218.205.203.83/g,'gkzq.com.cn') : username.replace(/121.30.232.162/g,'imserver');
			console.log(username);
	    // username = username.replace(/219.232.203.16/g,'gkzq.com.cn');
    ...
    joinRoom(roomJID, nickname, historyNum) {
        React.NativeModules.RNXMPP.joinRoom(roomJID, nickname, historyNum);
    }

    joinRoomNewThread(roomJID, nickname) {
        React.NativeModules.RNXMPP.joinRoomNewThread(roomJID, nickname);
    }

    sendRoomMessage(message, roomJID) {
        React.NativeModules.RNXMPP.sendRoomMessage(roomJID, message);
    }
