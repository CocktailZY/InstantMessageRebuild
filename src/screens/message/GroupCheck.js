...
export default class GroupCheck extends Component {
	...
	componentDidMount() {
		//update消息页面未读条数
/*		Sqlite.updateTalker(Global.loginUserInfo.userId, Global.loginUserInfo.userId, 0, true, false, false, () => {
	...
	getAuditMember = (pageNum) => {
		...
		FetchUtil.netUtil(Path.auditMembersList, params, 'POST', this.props.navigation, Global.basicParam, (data) => {
            if(data =="tip"){
                // this._toast('设置完成失败！');
                this.refs.toast.show('获取审核列表失败！', DURATION.LENGTH_SHORT);
                    ...
                    if (data.status) {
                        if (data.data == 'done') {
                            this.setState({
                                ...
                                tipMsg:'该成员已被其他管理员操作'//alert提示信息
                            });
                            this.touchLock = true;
                            this.getAuditMember(1);
													  this.getNotification();//通知数
                        } else {
											...
											if (error) {
												...
												PCNoticeUtil.pushPCNotification(item.roomJid,
													...
													{uuId:this.state.uuid,ticket:this.state.ticket,userId:Global.loginUserInfo.userId
														...
														XMPP.XMPPSetMember({'memberJid': item.applicantJid, 'roomJid': item.roomJid,'uuid':UUIDUtil.getUUID().replace(/\-/g, '')});
